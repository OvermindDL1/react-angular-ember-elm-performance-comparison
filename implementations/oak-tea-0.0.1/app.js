...
function subscriptions(model) {
  ...
  return Tea_time.every(500, function (t) {
              return /* Time */[t];
            });
}

function view(model) {
  return /* Text */Block.__(0, [Pervasives.string_of_float(model)]);
}
...
function programLoop(update, view, subscriptions, initModel, param) {
  if (param) {
    ...
    return function (callbacks) {
      ...
      var handleSubscriptionChange = function (model) {
        ...
        oldSub[0] = Tea_sub.run(callbacks[0][/* enqueue */0], oldSub[0], newSub);
        return /* () */0;
      };
...
function program(param, pnode, flags) {
  ...
  var pumpInterface = programLoop(param[/* update */1], param[/* view */2], param[/* subscriptions */3], initModel, opnode);
...
function run(enqueue, param) {
  if (typeof param === "number") {
    ...
    return Curry._1(param[0], enqueue);
  }
  else {
...
function registration(key, enableCall, disableCall) {
...
function run(enqueue, oldSub, newSub) {
  if (typeof oldSub === "number") {
    ...
    var aux = function (_idx, _oldList, _newList) {
      while(true) {
        ...
        if (oldList) {
          if (newList) {
            run(enqueue, oldList[0], newList[0]);
            return /* () */0;
          }
          else {
            _newList = oldList[1];
...
function every(interval, tagger) {
  var key = "" + interval;
  var state = [/* None */0];
  var enableCall = function (enqueue) {
    state[0] = /* Some */[Web_window.setInterval(function () {
            return Curry._1(enqueue, Curry._1(tagger, Date.now()));
          }, interval)];
    return /* () */0;
  };
  var disableCall = function () {
    var match = state[0];
    if (match) {
      var id = match[0];
