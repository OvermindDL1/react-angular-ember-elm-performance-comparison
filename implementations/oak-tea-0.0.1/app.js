...
var Main_todo = require("./main_todo");

var load = Main_todo.main(document.getElementById("content"), /* () */0);
    ...
    switch (param.tag | 0) {
      ...
      case 1 : 
          ...
          return /* tuple */[
                  ...
                  editing ? Tea_html_cmds.focus("todo-" + id) : /* None */0
                ];
      case 2 : 
...
function programStateWrapper(initModel, pump, shutdown) {
  ...
  var handler = function (msg) {
    var newModel = Curry._2(pumperInterface[/* handleMsg */0], model[0], msg);
    model[0] = newModel;
    return /* () */0;
  };
  ...
  var pi_requestShutdown = function () {
    Curry._1(shutdown, model[0]);
    Curry._1(pumperInterface[/* shutdown */1], /* () */0);
  ...
  if (param) {
    ...
    return function (callbacks) {
      ...
      var handler = function (model, msg) {
        ...
        Tea_cmd.run(callbacks[0][/* enqueue */0], match[1]);
        latestModel[0] = newModel;
        scheduleRender(/* () */0);
        return newModel;
      };
      var handlerShutdown = function () {
        nextFrameID[0] = /* None */0;
...
function program(param, pnode, flags) {
  ...
  var pumpInterface = programLoop(param[/* update */1], param[/* view */2], initModel, opnode);
  return programStateWrapper(initModel, pumpInterface, param[/* shutdown */3]);
}
...
function run(enqueue, param) {
  if (typeof param === "number") {
    ...
    return Curry._1(cb, enqueue);
  }
  else {
    return List.fold_left(function (_, cmd) {
                return run(enqueue, cmd);
              }, /* () */0, param[0]);
  }
}
...
function on(typ, $staropt$star, cb) {
  var key = $staropt$star ? $staropt$star[0] : "";
  return Vdom.on(typ, /* Some */[key], cb);
}

function onInput($staropt$star, msg) {
  var key = $staropt$star ? $staropt$star[0] : "";
  return on("input", /* Some */[key], function (ev) {
              var match = ev.target;
              if (match !== undefined) {
...
function onClick($staropt$star, msg) {
  var key = $staropt$star ? $staropt$star[0] : "";
  return on("click", /* Some */[key], function () {
              return /* Some */[msg];
            });
}
...
function onDoubleClick($staropt$star, msg) {
  var key = $staropt$star ? $staropt$star[0] : "";
  return on("dblclick", /* Some */[key], function () {
              return /* Some */[msg];
            });
}
...
function onBlur($staropt$star, msg) {
  var key = $staropt$star ? $staropt$star[0] : "";
  return on("blur", /* Some */[key], function () {
              return /* Some */[msg];
            });
}
...
function onFocus($staropt$star, msg) {
  var key = $staropt$star ? $staropt$star[0] : "";
  return on("focus", /* Some */[key], function () {
              return /* Some */[msg];
            });
}
...
function focus(id) {
  return /* EnqueueCall */Block.__(1, [function () {
              ...
              if (Js_primitive.js_is_nil_undef(match)) {
                console.log("Attempted to focus a non-existant element of: " + id);
                return /* () */0;
              }
              else {
                var ecb = function () {
...
function on(name, $staropt$star, cb) {
  var key = $staropt$star ? $staropt$star[0] : "";
  ...
  while(true) {
    ...
    if (oldVNodes) {
          ...
          switch (oldVnode.tag | 0) {
                        ...
                        if (newKey === "") {
                          ...
                          if (exit$2 === 3) {
                            if (oldUnique === newUnique) {
                              ...
                              var childChildren$2 = child$3.childNodes;
                              patchVNodesOnElems_Properties(callbacks, child$3, oldProperties, newProperties);
                              patchVNodesOnElems(callbacks, child$3, childChildren$2, 0, oldChildren, newChildren);
                              _newVNodes = newRest$1;
                              _oldVNodes = oldRest$1;
                              _idx = idx + 1 | 0;
                              continue ;
            ...
            case 2 : 
                ...
                if (typeof match$2 === "number") {
                  ...
                  var newRest$2 = newVNodes[1];
                  var newCache = match$2[2];
                  var newGen = match$2[1];
                  var newKey$1 = match$2[0];
                  var oldRest$2 = oldVNodes[1];
                  var oldCache = oldVnode[2];
                  if (oldVnode[0] === newKey$1) {
                    newCache[0] = oldCache[0];
                    _newVNodes = newRest$2;
                    ...
                    var exit$3 = 0;
                    if (oldRest$2) {
                      var match$3 = oldRest$2[0];
                      if (typeof match$3 === "number") {
                        exit$3 = 3;
                      }
                      else if (match$3.tag === 2) {
                        if (match$3[0] === newKey$1) {
                          var oldChild$1 = elems[idx];
                          elem.removeChild(oldChild$1);
                          _newVNodes = newRest$2;
                          _oldVNodes = oldRest$2[1];
                          continue ;
                          
                        }
                        else {
                          var oldVdom = oldCache[0];
      ...
      if (exit$1 === 2) {
        ...
        var oldRest$3 = oldVNodes[1];
        if (typeof match$4 === "number") {
          var child$5 = elems[idx];
          var newChild = document.createComment("");
          Web_node.insertBefore(elem, newChild, child$5);
          elem.removeChild(child$5);
          _newVNodes = newVNodes[1];
          _oldVNodes = oldRest$3;
          _idx = idx + 1 | 0;
          continue ;
          ...
          var newChild$1 = Web_document.createElementNsOptional(match$4[0], match$4[1]);
          patchVNodesOnElems_Properties(callbacks, newChild$1, /* [] */0, match$4[4]);
          var childChildren$3 = newChild$1.childNodes;
          patchVNodesOnElems(callbacks, newChild$1, childChildren$3, 0, /* [] */0, match$4[5]);
          Web_node.insertBefore(elem, newChild$1, oldChild$2);
          ...
          var child$6 = elems[idx];
          var newChild$2 = document.createTextNode(match$4[0]);
          Web_node.insertBefore(elem, newChild$2, child$6);
          elem.removeChild(child$6);
          _newVNodes = newVNodes[1];
          _oldVNodes = oldRest$3;
          _idx = idx + 1 | 0;
          continue ;
      ...
      if (typeof match$5 === "number") {
        var newChild$3 = document.createComment("");
        ...
        switch (match$5.tag | 0) {
          case 0 : 
              var newChild$4 = document.createTextNode(match$5[0]);
              elem.appendChild(newChild$4);
              _newVNodes = newVNodes[1];
              _oldVNodes = /* [] */0;
              _idx = idx + 1 | 0;
              continue ;
              case 1 : 
              var newChild$5 = Web_document.createElementNsOptional(match$5[0], match$5[1]);
              patchVNodesOnElems_Properties(callbacks, newChild$5, /* [] */0, match$5[4]);
              var childChildren$4 = newChild$5.childNodes;
              patchVNodesOnElems(callbacks, newChild$5, childChildren$4, 0, /* [] */0, match$5[5]);
    ...
    if (exit === 1) {
      ...
      var vdom$1 = Curry._1(match$6[1], /* () */0);
      match$6[2][0] = vdom$1;
      _newVNodes = /* :: */[
        vdom$1,
        newVNodes[1]
